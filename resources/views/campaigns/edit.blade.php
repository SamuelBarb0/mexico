...
<div class="bg-white rounded-lg shadow-md p-6">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Editar Campaña</h2>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="waba_account_id" id="waba_account_id" required
                    ...
                    @foreach($wabaAccounts as $waba)
                        <option value="{{ $waba->id }}" {{ old('waba_account_id', $campaign->waba_account_id) == $waba->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="type" id="type" required
                    ...
                    <option value="drip" {{ old('type', $campaign->type) === 'drip' ? 'selected' : '' }}>Drip</option>
                    <option value="trigger" {{ old('type', $campaign->type) === 'trigger' ? 'selected' : '' }}>Trigger</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="status" id="status"
                    ...
                    <option value="scheduled" {{ old('status', $campaign->status) === 'scheduled' ? 'selected' : '' }}>Programada</option>
            ...
            <div>
                ...
                <input type="datetime-local" name="started_at" id="started_at"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                    value="{{ old('started_at', $campaign->started_at ? $campaign->started_at->format('Y-m-d\TH:i') : '') }}">
            </div>

            <div class="md:col-span-2">
                <label for="description" class="block text-sm font-medium text-gray-700">Descripción</label>
                <textarea name="description" id="description" rows="2"
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $campaign->description) }}</textarea>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="message_template" id="message_template" rows="4" required
                    ...
                    placeholder='{"text": "¡Hola {name}! Tenemos ofertas especiales", "buttons": ["Ver Ofertas"]}'>{{ old('message_template', $campaign->message_template ? json_encode($campaign->message_template, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '') }}</textarea>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="target_audience" id="target_audience" rows="3"
                    ...
                    placeholder='{"tags": ["vip", "premium"], "status": "active"}'>{{ old('target_audience', $campaign->target_audience ? json_encode($campaign->target_audience, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '') }}</textarea>
